...
```
Frontend (React + Vite)
├── PortfolioForm        — Manual stock entry
├── RiskDashboard        — Risk visualizations & charts
├── StockAnalysis        — Individual stock recommendations
├── ChatWindow           — AI chat assistant
├── AuthModal            — Firebase login/signup
└── SavedPortfolios      — Persistent portfolio storage
         │
         │  REST API
         ▼
...
│   └── Market Analyzer Agent (Web Search)
│
├── Utility Layer
│   ├── Price Fetcher (Alpha Vantage API)
│   ├── Risk Calculations (Volatility, HHI, Correlation)
│   └── Portfolio Validator (CSV parsing, input validation)
│
└── External APIs
    ├── Google Gemini API
    └── Alpha Vantage API
...
Optionally, create a `.env` file to set the API URL if the backend isn't on `localhost:8000`:
...
│   │   ├── calculations.py             # Risk math (volatility, HHI, etc.)
...
| `ALPHA_VANTAGE_API_KEY` | ❌ No | Alpha Vantage key for live stock prices (falls back to mock data) |
| `FIREBASE_PROJECT_ID` | ❌ No | Firebase project ID (for server-side Firebase) |
